...
Engineering abstraction for simulation; not a clinical or diagnostic model.
"""
...
class CardiovascularModel:
    ...
    def step(self, dt: float, activity: float = 0.0) -> dict[str, float]:
        ...
        gain = min(1.0, max(0.0, float(dt)))
        target_hr = 65.0 + 55.0 * demand
        self.heart_rate += (target_hr - self.heart_rate) * min(1.0, gain * 0.5)
        ...
        self.arterial_pressure += (target_pressure - self.arterial_pressure) * min(1.0, gain * 0.8)
        self.venous_return += (self.arterial_pressure - self.venous_return) * min(1.0, gain * 0.6)
        return {
